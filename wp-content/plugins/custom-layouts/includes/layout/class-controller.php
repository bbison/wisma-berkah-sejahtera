...
class Controller {
	...
	public function render( $return = false ){
		...
		if ( isset( $this->settings['item_spacing'] ) ) {
			...
			$grid_gap    = array( 'column' => $spacing_gap, 'row' => $spacing_gap );
		...
		} else if ( ! isset( $this->settings['grid_gap'] ) ) {
			$this->settings['grid_gap'] = array( 'column' => '10px', 'row' => '10px' );
		}
		...
		// Add the gap.
		ob_start();
		?>--cl-layout-gap-c: <?php echo $column_gap; ?>;--cl-layout-gap-r: <?php echo $row_gap; ?>;<?php
		$css = ob_get_clean();
